...
export default function MarketsPage() {
  ...
  // Real balance from API (null = loading)
  ...
  // Fetch real balance & open bets from API
  ...
  useEffect(() => {
    if (isUserLoaded) fetchProfile();
  }, [isUserLoaded, isSignedIn, fetchProfile]);
    ...
    setPositions((prev) => {
      ...
      prev.forEach((pos) => {
        if (pos.windowEnd <= marketEnd) {
          ...
          if (userWon) {
            setRealBalance((bal) => (bal ?? 0) + payout);
  ...
  // One-tap Buy execution handler — real API
  const handleOneTapBuy = async (amount: number) => {
    if (!isSignedIn) {
      showToast("⚠️ Please log in to trade.");
      return;
    }
    if (realBalance !== null && amount > realBalance) {
      showToast("⚠️ Insufficient balance.");
      return;
    }
    if (!btcPrice || btcPrice === 0) {
      showToast("⚠️ Price not loaded yet.");
      return;
    }

    const prediction = selectedSide === "YES" ? "up" : "down";
    ...
    try {
      ...
      if (res.ok) {
        ...
        setRealBalance((bal) => (bal ?? 0) - amount);
        ...
        const newPos: ActivePosition = {
          id: data.bet?.id ?? Math.random().toString(),
          ...
          {isSignedIn ? (
            <div className="flex items-center gap-2 bg-brand-dark text-white px-3.5 py-1.5 rounded-full shadow-sm">
              ...
              <span className="text-xs font-bold tabular-nums">
                ₹{realBalance !== null ? realBalance.toLocaleString("en-IN", { minimumFractionDigits: 2 }) : "—"}
              </span>
              <Link href="/deposit" className="text-[9px] text-brand-green bg-brand-green/10 px-1.5 rounded-full font-bold uppercase hover:bg-brand-green/20 transition-colors">
                Add
              </Link>
            </div>
          ) : (
            <Link href="/login" className="text-xs font-bold text-brand-green bg-brand-green/10 hover:bg-brand-green/20 px-3 py-1.5 rounded-full transition-colors">
              Log in
            </Link>
          )}
        </div>
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 flex flex-col gap-6">
        ...
        <div className="fixed bottom-0 left-0 right-0 z-30 bg-white border-t border-brand-border p-4 shadow-[0_-10px_30px_rgba(0,0,0,0.06)] rounded-t-2xl md:relative md:bottom-auto md:left-auto md:right-auto md:z-auto md:border md:rounded-xl md:p-5 md:shadow-none flex flex-col gap-4">
          ...
          <div>
            ...
            {/* Error state */}
            {realBalance !== null && realBalance < 5 && (
            ...
            <div className="flex justify-between text-[11px] font-bold text-gray-400 mt-3 pt-3 border-t border-brand-border">
              <span>Balance</span>
              <span className="text-brand-dark tabular-nums">₹{realBalance !== null ? realBalance.toLocaleString("en-IN", { minimumFractionDigits: 2 }) : "—"}</span>
            </div>
          </div>
        </div>
